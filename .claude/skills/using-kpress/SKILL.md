...
Size the document through the one base knob, never per size: set
`--kpress-host-font-size-base` (any length, e.g. `17px`) once on `:root` and every
kpress font size, bullet, and label scales proportionally, independent of the browser
root font size.
Do not override `--kpress-font-size-normal` or other ramp tokens — they derive from the
base. The `:root` hook also reaches body-appended tooltips; a `.kpress`-scoped override
does not.
