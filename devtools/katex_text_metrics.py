...
-- the serif set as the object's own face keys, the sans set nested under ``sans`` --
and ``katex-init.js`` installs one set or the other per rendered node with
...
def sans_font(weight: int, style: str) -> str:
    ...
    so this tool keeps reading fonts by file name as it does for PT Serif.
    """
...
def render_asset(tables: Mapping[str, MetricTable], sans_tables: Mapping[str, MetricTable]) -> str:
    ...
    `katex-init.js` installs one or the other per rendered node.
    """
