...
`.tab-button`, `summary`) and the reader’s sans reading face
...
has always assumed. Headings and the TOC are sans roles too and are deliberately left to
the serif composite in a serif document: both are set at the sans bold weight, where the
composite’s pinned regular weight would draw the mathematics a step lighter than the
words. Under the sans reading face the whole document is sans and the composite follows
it, headings included, since the alternative there is a serif expression in a sans
paragraph.
...
Under `@media print`, and declared last, the static `Source Sans 3` instances at the
...
Latin advance of the shipped instance, so one metric table is true of both.
...
runs after it. The role list it tests with `closest()` is the one the stylesheet scopes
on, and `tests/test_sans_math_face_css.py` pins the two copies together.
