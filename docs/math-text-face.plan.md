...
reason. A shipped subset now answers those code points instead.
...
- `kpr-asj4`, done: the quotation marks come from `KPress Quotes`, a 724-byte subset of
  Source Serif 4 over the six code points `LocalPunct` covered, generated by
  `devtools/subset_quotes.py` and leading the prose stack in its place.
  Borrowing Georgia was deliberate work, not an oversight, and its reason was never
  written down: at 18px PT Serif hangs its opening pair 2.25px above its closing pair
  and sets its curly doubles 16% wider than Georgia, where Georgia’s open and close are
  level to 0.03px. The fix keeps the reason and drops the borrowing: the shipped marks
  are level to 0.00px and 7% wider than Georgia’s, and they are the same marks on every
