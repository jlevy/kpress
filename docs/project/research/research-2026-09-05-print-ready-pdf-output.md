...
The central conclusion is that KPress should not choose between “the browser” and “a
typesetter” as if one executable can satisfy every output class.
It should define one static **print artifact contract** and support a small family of
explicit PDF quality profiles over it:

1. **Chromium reader PDF** as the broadly compatible, web-faithful baseline.
2. **Prince publication PDF** as the recommended optional path for sophisticated paged
   typography and, when required, standards-based press output.
...
4. switches to print media; and
5. calls `page.pdf()` with a named paper format and background printing enabled.
...
- `networkidle` does not prove that fonts, KaTeX, diagrams, custom elements, canvas
  redraws, or host-specific asynchronous work are ready.
...
- The [PDF contract tests](../../../tests/test_pdf_contract.py) mock the browser calls;
  they do not inspect a real PDF, its fonts, tags, links, page boxes, visual pages, or
  repeatability.
- The public design currently calls the output deterministic without distinguishing
  deterministic HTML, stable layout in a pinned environment, and byte-identical PDF.
...
- a dense one-hundred-cell atlas where raster downscaling quickly makes labels
  illegible; and
- KPress’s full-screen reading shell embedded in a custom page shell.

The Squares-specific print CSS documents two failures a generic renderer would not
discover:

1. KPress releases `.kpress-viewport` for pagination, but the consuming page also keeps
   `html, body` at fixed height with hidden overflow.
   Without a host override, Chromium prints one page and omits most of the report.
2. Canvas is a bitmap. Print CSS cannot recolor or vectorize it, so Squares listens for
   ...
   That makes interactive printing better, but it still produces raster graphics and
   relies on a timing-sensitive event.
...
Prince is not an obsolete browser-print hack.
It is a long-lived, actively maintained dedicated formatter; Prince 16.2 was released in
January 2026. Its current
[paged-media documentation](https://www.princexml.com/doc/paged/) covers named and
selected pages, running matter, footnotes, sidenotes, page floats, page groups,
recto/verso layouts, counters, and detailed break control.
...
Its recommendation is therefore an optional supported adapter and documented production
path, not a mandatory runtime dependency or silent replacement for Chromium.
...
| Prince | Strong with print-specific CSS | Limited, non-browser JS | Strong | Strong PDF/A, PDF/UA, PDF/X, color controls | Commercial CLI/container | Recommended publication and press adapter |
...
The common boundary must therefore be a static, self-contained print representation in a
known state.
...
KPress should stop using “deterministic PDF” as one undifferentiated promise:
...
**Decision:** Required first step and supported default, but insufficient as KPress’s
only long-term publication path.
...
Evolve PDF configuration conceptually toward:
...
- the report records profile, engine and version, browser revision or container digest,
  font manifest, input digest, page count, warnings, validation results, and output
  digest.
...
- all CSS, fonts, images, SVG, and supporting data local, pinned, and hashed;
...
For browser materialization, add a KPress readiness protocol.
It should wait for at least `document.fonts.ready`, successful image decoding, built-in
KaTeX and diagram completion, and registered host promises.
A host-facing hook could conceptually be `kpress.registerPrintTask(promise)` or a single
`preparePrint()` callback.
The exact API needs design, but it must have a timeout, component names, and actionable
failure output.
...
- fix or document page-level shell release so host `html/body` overflow cannot truncate
  the document;
- add the readiness and closed-asset protocol;
...
- control title, author, subject, language, creation/modification time, locale, and
  timezone; and
- add real-PDF tests instead of only browser-call mocks.
...
- Harden Chromium options, readiness, asset closure, metadata, diagnostics, and tests.
...
- Add print fallbacks for KPress math, diagrams, tabs, disclosures, and custom graphics.
...
- [ ] Open an implementation bead to harden the Chromium reader backend.
...
- [ ] Update claims in `kpress-design.md` so “deterministic” names the exact level being
  guaranteed.
...
The precise Prince-versus-WeasyPrint quality and compatibility ranking has medium
confidence until the proposed corpus is rendered.
Vendor documentation establishes available controls, not the quality of a specific
KPress document.
