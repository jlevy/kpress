...
def test_probe_optimizer_full_ok_when_npx_present() -> None:
...
def test_probe_optimizer_full_unavailable_when_npx_missing(
...
def test_probe_result_has_stable_shape() -> None:
    ...
    r2 = ProbeResult(status="fail", reason="npx_not_found")
...
def test_build_html_full_preflight_fails_before_writing(
    ...
    """When optimizer=full and npx is missing, build_html raises before writing output."""
    from kpress.publish import BuildOptions, build_html

    monkeypatch.setattr(shutil, "which", _stub_which)
    dest = tmp_path / "output" / "page.html"
    with pytest.raises(KPressMissingOptionalDependencyError, match=r"npx|html-minifier"):
...
def test_build_site_full_preflight_fails_with_untouched_output(
    ...
    """When optimizer=full and npx is missing, build_site raises with no output written."""
...
def test_build_html_none_mode_does_not_preflight(
    ...
    """optimizer=none does not require npx and writes output normally."""
    from kpress.publish import BuildOptions, build_html
...
def test_build_html_full_preflight_passes_and_builds(
    ...
    """optimizer=full with npx present preflights successfully and builds."""
    from kpress.publish import BuildOptions, build_html
