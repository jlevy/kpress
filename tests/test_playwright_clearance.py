...
The clearance is a cascade property: the narrow-band rule reserves
``max(1rem, var(--kpress-toc-toggle-clearance))`` on ``.kpress-doc`` and a
later same-specificity phone rule must preserve it while tightening the base
padding. Browserless DOM emulation does not evaluate container queries, so
the computed padding is asserted in real Chromium at one phone width and one
tablet width.
"""
    ...
    try:
        with sync_api.sync_playwright() as playwright:
            ...
            try:
                ...
                # Phone width: the tighter base padding must not undercut the
                # clearance reservation (the regression computed 8px here).
                phone = doc_metrics()
                assert phone["paddingLeft"] >= phone["clearance"]

                # Tablet width, still inside the floating-toggle band.
